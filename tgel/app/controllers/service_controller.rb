class ServiceController < ApplicationController
  ...
  def destroy_services
    get_xvars
    TgelService.delete_all "app_id=#{@xvars[:id]}"
  end
  def destroy_controllers
    get_xvars
    @app= App.find @xvars[:id]
    modules= @app.tgel_services.all :group=>'module'
    tt= ''
    modules.each do |m|
...
#    @xvars[:notice_svn]= notice
  ...
  def process_services
    ...
    f= "tgel/#{@app.code}/index.mm"
    xml= REXML::Document.new(File.read(f).gsub("\n","")).root
    TgelService.delete_all "app_id=#{@app.id}"
    TgelService.delete_all "app_id IS NULL"
    ...
    @services.each_element('node') do |m|
      ...
      m.each_element('node') do |s|
        ...
        TgelService.create :app_id=>@app.id,
          :module=>module_code,
